...
:since: v0.1.00
		...
		_return = self.get_url(_type, parameters)
...
:since:  v0.1.01
	...
	def get_url(self, _type, parameters):
	#
		"""
Adds a link.
...
:since:  v0.1.00
		...
		if ("__query__" in _return and len(_return['__query__']) > 0):
		#
			if (len(_return) == 1): _return = AbstractRequest.get_instance().iline_parse(InputFilter.filter_control_chars(_return['__query__']))
			else: del(_return['__query__'])
		#
		elif ("__request__" in _return and _return['__request__']):
		#
			request = AbstractRequest.get_instance()
